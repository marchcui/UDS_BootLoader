...
/* ISO 15765-2 7.3.3 */
#define NFA_N_TATYPE_PHY_VAL                218U //0xda
#define NFA_N_TATYPE_FUN_VAL                219U //0xdb
...
* NWS_MSG_TIM_t
*
* @note network layer timing  网络层时间参数
*/
typedef struct
{
  ...
  U32 uNAr;           /* Timeout between req FC and con FC *///Receiver端请求FC和确认FC之间的参数
}NWS_MSG_TIM_t;
...
typedef struct
{
  NWS_MSG_TX_t *pMSG_TX; //TP/网络层发送数据结构
  NWS_MSG_RX_t *pMSG_RX; //TP/网络层接收数据结构
  NWS_MSG_TIM_t *pMSG_TIM; //TP/网络层时间参数结构
} s_NWS_MSG;
...
/**
* uds & network layer configs UDS和网络层配置
*
* @note 
*/
typedef struct {
  U8 idxf;
  U8 idx;
  UDS_CANrxBuf_t *pCANrxBuf; //CAN接收buf结构
  UDS_CANtxBuf_t *pCANtxBuf; //CAN发送buf结构
  UDS_CONST_STORAGE ADDR_FORMAT_t *pAFR; //接收地址类型
  UDS_CONST_STORAGE ADDR_FORMAT_t *pAFT; //发送地址类型
  UDS_CONST_STORAGE s_NWS_MSG *pNWS_MSG; //TP/网络层消息结构
  UDS_CONST_STORAGE s_UDS_DSI *pUDS_DSI; //诊断服务接口结构
  U8 DSI_MAX;
} s_UDS_CFGS; //UDS配置结构
